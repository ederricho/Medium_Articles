---
title: '[Quick Demo] Explore Function'
...
# ----- Fake Data with NA Values -----
fakeData <- c(seq(1,1000,0.2), rep("NA",3*n), rep("N/A",3*n), rep(" ", 3*n))

# ----- Generate Dataframe -----
sample1 <- sample(fakeData, n, replace = T)
sample2 <- sample(fakeData, n, replace = T)
sample3 <- as.character(sample(fakeData, n, replace = T))
sample4 <- sample(fakeData, n, replace = T)
sample5 <- sample(fakeData, n, replace = T)
sample6 <- as.character(sample(fakeData, n, replace = T))
sample7 <- sample(fakeData, n, replace = T)
df <- data.frame(sample1, sample2, sample3, sample4, sample5, sample6, sample7)
head(df)
```

# Run Explore
```{r}
explore <- function(df, imputation = "Mean", printTest = TRUE){
  # ----- Empty Vectors -----
  tTestNames <- c()
  tTestResults <- c()
  testPrint <- c()
